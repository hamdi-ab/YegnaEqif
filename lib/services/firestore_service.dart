...
import '../features/transactions/model/transaction.dart' as transaction;
import '../features/budget/model/budget.dart';
...
class FirestoreService {
  ...
  Future<void> addCategory(
      String userId, String name, IconData icon, Color color) async {
  ...
  Future<void> updateCategory(
      String userId, String categoryId, Category updatedCategory) async {
  ...
  Future<void> addTransaction(
      String userId, transaction.Transaction transaction) async {
    await _firestore
        .collection('users')
        .doc(userId)
  ...
  Future<void> updateTransaction(String userId, String transactionId,
      transaction.Transaction updatedTransaction) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('transactions')
  ...
  Future<void> updateBudget(
      String userId, String budgetId, Budget updatedBudget) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('budgets')
  ...
  Future<void> updateDebt(
      String userId, String debtId, Debt updatedDebt) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('debts')
        .doc(debtId)
        .update(updatedDebt.toFirestore());
  }

  // Bank Account Methods
  Future<void> addBankAccount(
      String userId, BankAccountCardModel bankAccount) async {
    final docRef = await _firestore
        .collection('users')
        .doc(userId)
  ...
  Future<void> updateBankAccount(String userId, String bankAccountId,
      BankAccountCardModel updatedBankAccount) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('bankAccounts')
        .doc(bankAccountId)
        .update(updatedBankAccount.toMap());
  }
  ...
  Future<void> updateCashCard(
      String userId, String cashCardId, CashCardModel updatedCashCard) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('cashCards')
        .doc(cashCardId)
        .update(updatedCashCard.toMap());
  }

  // Total Balance Card Methods (local storage handled separately)
  Future<void> addTotalBalanceCard(
      String userId, TotalBalanceCardModel totalBalanceCard) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('totalBalanceCards')
        .add(totalBalanceCard.toMap());
  }

  Future<List<TotalBalanceCardModel>> fetchTotalBalanceCards(
      String userId) async {
    final snapshot = await _firestore
        .collection('users')
        .doc(userId)
  ...
  Future<void> removeTotalBalanceCard(
      String userId, String totalBalanceCardId) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('totalBalanceCards')
        .doc(totalBalanceCardId)
        .delete();
  }

  Future<void> updateTotalBalanceCard(String userId, String totalBalanceCardId,
      TotalBalanceCardModel updatedTotalBalanceCard) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('totalBalanceCards')
        .doc(totalBalanceCardId)
        .update(updatedTotalBalanceCard.toMap());
  }
}
